...
wait_for_mongo() {
  local container=$1
  echo -n "⏳ Waiting for $container to become ready..."
  for i in {1..60}; do
    if docker exec "$container" mongosh --quiet --eval "db.adminCommand({ping:1})" &>/dev/null; then
      echo " OK"
      return 0
    fi
    sleep 2
...
### ---------------------------------------------------------
echo ""
echo "=== MongoDB ReplicaSet Bootstrap ==="
echo "How many replica nodes? Example: 1, 2, 3"
read -rp "Replica count (default 1): " REPLICAS
REPLICAS=${REPLICAS:-1}

read -rp "Admin username (default: admin): " MONGO_USER
MONGO_USER=${MONGO_USER:-admin}

read -rsp "Admin password (leave empty for auto-gen): " MONGO_PASS
echo ""
if [[ -z "$MONGO_PASS" ]]; then
  MONGO_PASS=$(generate_password)
  echo "Generated password: $MONGO_PASS"
fi
...
echo "✔ Keyfile created → secrets/mongodb-keyfile"
...
cat >> docker-compose.yml <<EOF
  mongo$i:
    ...
    restart: always
    command: ["mongod", "--replSet", "$DEFAULT_REPLSET", "--bind_ip", "0.0.0.0", "--keyFile", "/etc/secrets/mongo-keyfile"]
    environment:
...
### ---------------------------------------------------------
### STEP 5 — Wait for all members
### ---------------------------------------------------------
for ((i=1; i<=REPLICAS; i++)); do
  wait_for_mongo "mongo$i"
done

### ---------------------------------------------------------
### STEP 6 — Build replica set config
### ---------------------------------------------------------
...
for ((i=1; i<=REPLICAS; i++)); do
  CONFIG+=" { _id: $((i-1)), host: \"mongo$i:27017\" },"
done

CONFIG="${CONFIG%,} ] }"

docker exec mongo1 mongosh --quiet --eval "rs.initiate($CONFIG)" || true
sleep 3

### ---------------------------------------------------------
### STEP 7 — Create admin/root user
### ---------------------------------------------------------
echo "▶ Creating admin user..."

docker exec mongo1 mongosh --quiet --eval "
  if (!db.getSiblingDB('admin').getUser('$MONGO_USER')) {
...
echo "============================================"
echo "Admin Username : $MONGO_USER"
echo "Admin Password : $MONGO_PASS"
echo ""
echo "Connection String:"
echo "mongodb://$MONGO_USER:$MONGO_PASS@localhost:27017/?replicaSet=$DEFAULT_REPLSET"
echo ""
echo "All members:"
for ((i=1; i<=REPLICAS; i++)); do
  echo " - mongo$i → port $((DEFAULT_PORT + i - 1))"
done
echo ""
